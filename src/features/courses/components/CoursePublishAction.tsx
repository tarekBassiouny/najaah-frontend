...
import { useExecuteContentPublishing, useAgentExecution } from "@/features/agents";
...
export function CoursePublishAction({ course }: CoursePublishActionProps) {
  ...
  const { mutate: executePublish, isPending: isStarting } = useExecuteContentPublishing();
  ...
  const progress = execution?.stepsCompleted
    ? (execution.stepsCompleted.filter((s) => s.status === "completed").length / 6) * 100
    : 0;
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {executionId && (
            <div className="space-y-4 py-4">
              ...
              {execution?.stepsCompleted && execution.stepsCompleted.length > 0 && (
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1.5">
                    {execution.stepsCompleted.map((step, index) => (
                      ...
                      >
                        ...
                        <span className={step.status === "completed" ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400"}>
                          {step.name}
                        </span>
                        {step.status === "completed" && (
                        ...
                        {step.status === "failed" && (
                          <Badge variant="error" className="ml-auto text-xs">Failed</Badge>
                        )}
                      </li>
                    ))}
                  </ul>
