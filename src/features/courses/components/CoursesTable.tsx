    ...
    <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                  ...
                  <TableRow>
                    <TableCell colSpan={4}>
                      <div className="flex flex-col items-center gap-2 py-10 text-center">
                        ...
                        <p className="text-sm text-dark-5 dark:text-dark-4">
                          There are no courses matching the current criteria.
                        </p>
                      </div>
                    </TableCell>
                  </TableRow>
                ...
                : items.map((course) => (
                  <TableRow key={course.id}>
                    ...
                    <TableCell className="px-3 py-2 text-sm">
                      {course.status ? (
                        <span className={getBadgeClass(course.status)}>
                          {formatBadgeLabel(course.status)}
                        </span>
                      ) : (
                        "—"
                      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-sm text-dark-5 dark:text-dark-4">
          Page {data?.page ?? page} of {lastPage}
        </div>
